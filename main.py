...
class Player(pygame.sprite.Sprite):
    ...
    def collide(self, yvel, platforms):
        ...
        for p in platforms:
            if pygame.sprite.collide_rect(self, p):
                ...
                if isinstance(p, Orb) and (keys[pygame.K_UP] or keys[pygame.K_SPACE] or player.should_jump()): # TODO figure out how this works
    ...
    # def should_jump(self,time):
    #     return False


class Bot(Player):
    def __init__(self, image, platforms, pos, *groups,jumps):
        super().__init__(image, platforms, pos, *groups)
        self.jumps=jumps #array of tuples are tuples immutable again nvmd 2d array why tf is it 2d just one d and sorted
    
    def should_jump(self):
        
        arr1= np.where(self.jumps == self.counter-1)
        arr2= np.where(self.jumps == self.counter)
        arr3= np.where(self.jumps == self.counter+1)
        return len(arr1[0])>0  or len(arr2[0])>0 or len(arr3[0])>0
        # return True
...
player = Bot(avatar, elements, (150, 150), player_sprite, jumps= np.array([60]))
